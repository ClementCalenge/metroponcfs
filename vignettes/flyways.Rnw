...
XXThe numbers are incorrect here (previous model with uniform prior on
the logit scale). I am waiting for the calculations to end.

Note that here, the two DIC are similar, though the model accounting
for time is slightly better. The DIC difference is too small to
